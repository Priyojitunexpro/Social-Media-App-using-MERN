...
const Profile  = ()=>{
    ...
    const likePost = (id)=>{
          ...
          .then(result=>{
            ...
            const newData = data.map(item=>{
                if(item._id==result._id){//the file to store object asssociated with delete account
                    return result
                }else{
                    return item
                }
    ...
    const unlikePost = (id)=>{
          ...
          .then(result=>{
            ...
            const newData1 = mypics.map(item=>{//the file to store object asssociated with delete account
                if(item._id==result._id){//the file to store object asssociated with delete account
                    return result
                }else{
                    return item
    ...
    const makeComment = (text,postId)=>{
          fetch('/comment',{
              ...
              headers:{
                  "Content-Type":"application/json",
                  "Authorization":"Bearer "+localStorage.getItem("jwt")//the file to store object asssociated with delete account
              },
              body:JSON.stringify({
                  postId,
                  text
    ...
    const deleteComment = (postid, commentid) => {
        ...
        fetch(`/deletecomment/${postid}/${commentid}`, {//the file to store object asssociated with delete account
          method: "delete",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("jwt"),
          },
          ...
          .then((result) => {
            ...
            setData(newData);
            setPics(newData1)
          }).then(result=>{//the file to store object asssociated with delete account
    ...
    useEffect(()=>{
       if(image){
        const data = new FormData()//the file to store object asssociated with delete account
    ...
    const updatePhoto = (file)=>{//the file to store object asssociated with delete account
        setImage(file)
    }
   ...
   return (
    ...
    <div className="ll mur" style={{display:"flex",justifyContent:"space-between",width:"108%"}}>
            ...
            </div>    
           {
                  ...
                  : <Fragment>
                  {//the file to store object asssociated with delete account//the file to store object asssociated with delete account
        ...
        { (item.postedBy._id===state._id  || record.postedBy._id===state._id)
                             ...
                             && (
                                          <i
                                            ...
                                            onClick={() => deleteComment(item._id, record._id)}//the file to store object asssociated with delete account
                                          >
                                            delete
                                          </i>
                                        )}
                                ...
                                <form onSubmit={(e)=>{//the file to store object asssociated with delete account
                                    e.preventDefault()
                                    makeComment(e.target[0].value,item._id)//the file to store object asssociated with delete account
                                    e.target[0].value=""
                                }}>
