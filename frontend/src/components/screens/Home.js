...
const Home  = (props) =>{
    ...
    const likePost = (id)=>{
          fetch('/like',{
              ...
              body:JSON.stringify({//we are stringify ing json object
                  postId:id//here we are storing the id of the post
              })
          }).then(res=>res.json())
          .then(result=>{
    ...
    const makeComment = (text,postId)=>{
          fetch('/comment',{//the file to store object asssociated with other user's comments
              method:"put",// method as post
              headers:{
                  "Content-Type":"application/json",//type of the comment is application.json
                  "Authorization":"Bearer "+localStorage.getItem("jwt")
              },
              body:JSON.stringify({
                  postId,
          ...
          .then(result=>{
              console.log(result)//we are printing the result in console
              const newData = data.map(item=>{
                if(item._id==result._id){
                    return result
                }else{
            ...
            setData(newData)
              ...
              console.log(err)//we are printing the undesired error in console
          })
    }
